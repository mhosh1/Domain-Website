...
app.post('/api/waitlist', async (req, res) => {
    try {
        ...
        db.get('SELECT email FROM waitlist WHERE email = ?', [normalizedEmail], async (err, row) => {
            ...
            db.run('INSERT INTO waitlist (email) VALUES (?)', [normalizedEmail], async function(err) {
                if (err) {
...
app.get('/api/admin/waitlist', (req, res) => {
    db.all('SELECT id, email, created_at, notified FROM waitlist ORDER BY created_at DESC', (err, rows) => {
        if (err) {
            return res.status(500).json({ error: 'Database error' });
        }
        res.json(rows);
